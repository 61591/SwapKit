...
const ethGasChains = [Chain.Arbitrum, Chain.Aurora, Chain.Base, Chain.Ethereum, Chain.Optimism, Chain.Unichain] as const;
...
export function isGasAsset({ chain, symbol }: { chain: Chain; symbol: string }) {
  return match(chain)
    ...
    .with(Chain.Botanix, () => symbol === "BTC")
    .with(Chain.Core, () => symbol === "CORE")
...
export const getCommonAssetInfo = (assetString: CommonAssetString) => {
  ...
  const commonAssetInfo = match(assetString.toUpperCase())
    ...
    .with(Chain.Corn, "CORN.BTCN", () => ({ decimal, identifier: "CORN.BTCN" }))
