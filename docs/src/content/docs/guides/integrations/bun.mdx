---
title: Bun + React
description: A guide to integrate SwapKit with Bun and React applications.
---

This guide will help you integrate SwapKit into your Bun-powered React application. Bun is a fast JavaScript runtime that includes a bundler, test runner, and package manager.

## Installation

Install SwapKit using Bun:

```bash
bun add @swapkit/sdk @swapkit/ui
```

## Project Setup

Create a new Bun + React project:

```bash
bun create react my-swapkit-app
cd my-swapkit-app
```

## Basic Configuration

Create a `bunfig.toml` file for Bun configuration:

```toml
[serve.static]
plugins = ["bun-plugin-tailwind"]
env = "BUN_PUBLIC_*"
```

## Server Setup

Create a server file to serve your React application:

```tsx
// @noErrorValidation
// src/server.tsx
import { serve } from "bun";

const PORT = process.env.PORT || 3000;

serve({
  port: PORT,
  async fetch(req) {
    const url = new URL(req.url);
    
    // Serve static files
    if (url.pathname === "/") {
      return new Response(Bun.file("pages/index.html"));
    }
    
    // Handle API routes if needed
    if (url.pathname.startsWith("/api/")) {
      // Add your API logic here
    }
    
    // Default 404
    return new Response("Not Found", { status: 404 });
  },
});

console.log(`Server running at http://localhost:${PORT}`);
```

## React Application Setup

Create your main React application with SwapKit:

```tsx
// @noErrorValidation
// src/App.tsx
import { useState } from 'react';
import { createSwapKit } from '@swapkit/sdk';
import { Chain, WalletOption } from '@swapkit/helpers';

const swapKit = createSwapKit();

export function App() {
  const [isConnected, setIsConnected] = useState(false);
  const [address, setAddress] = useState<string>('');

  const connectWallet = async () => {
    try {
      const connectedChains = await swapKit.connectWallet(
        WalletOption.METAMASK,
        [Chain.Ethereum]
      );
      
      if (connectedChains.length > 0) {
        setIsConnected(true);
        const ethAddress = await swapKit.getAddress(Chain.Ethereum);
        setAddress(ethAddress);
      }
    } catch (error) {
      console.error('Failed to connect wallet:', error);
    }
  };

  const disconnectWallet = () => {
    swapKit.disconnectWallet(Chain.Ethereum);
    setIsConnected(false);
    setAddress('');
  };

  return (
    <div className="p-8">
      <h1 className="text-3xl font-bold mb-4">SwapKit + Bun</h1>
      {!isConnected ? (
        <button 
          onClick={connectWallet}
          className="px-4 py-2 bg-blue-500 text-white rounded"
        >
          Connect Wallet
        </button>
      ) : (
        <div>
          <p className="mb-4">Connected: {address}</p>
          <button 
            onClick={disconnectWallet}
            className="px-4 py-2 bg-red-500 text-white rounded"
          >
            Disconnect
          </button>
        </div>
      )}
    </div>
  );
}
```

## Entry Point

Create the entry point for your React application:

```tsx
// @noErrorValidation
// pages/index.tsx
import { createRoot } from 'react-dom/client';
import { App } from '../src/App';
import '../styles/index.css';

const container = document.getElementById('root');
if (container) {
  const root = createRoot(container);
  root.render(<App />);
}
```

## HTML Template

Create an HTML template:

```html
<!-- pages/index.html -->
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>SwapKit + Bun</title>
</head>
<body>
  <div id="root"></div>
  <script type="module" src="/pages/index.tsx"></script>
</body>
</html>
```

## Build Configuration

Create a build script for production:

```typescript
// @noErrorValidation
// build.ts
import { build } from "bun";

await build({
  entrypoints: ["./pages/index.tsx"],
  outdir: "./dist",
  target: "browser",
  minify: true,
  splitting: true,
  sourcemap: "external",
});
```

## Package.json Scripts

Add useful scripts to your `package.json`:

```json
{
  "scripts": {
    "dev": "bun --hot src/server.tsx",
    "build": "bun run build.ts",
    "start": "NODE_ENV=production bun src/server.tsx"
  }
}
```

## Using SwapKit UI Components

If you're using `@swapkit/ui`, you can leverage pre-built components:

```tsx
// @noErrorValidation
// src/Widget.tsx
import { SwapWidget } from '@swapkit/ui';
import { createSwapKit } from '@swapkit/sdk';

const swapKit = createSwapKit();

export function Widget() {
  return (
    <SwapWidget 
      swapKit={swapKit}
      theme="dark"
    />
  );
}
```

## Environment Variables

Bun uses `BUN_PUBLIC_` prefix for client-side environment variables:

```bash
# .env
BUN_PUBLIC_SWAPKIT_API_KEY=your_api_key_here
BUN_PUBLIC_WALLETCONNECT_PROJECT_ID=your_project_id_here
```

Access them in your code:

```typescript
// @noErrorValidation
import { SKConfig } from '@swapkit/helpers';

if (process.env.BUN_PUBLIC_SWAPKIT_API_KEY) {
  SKConfig.setApiKey('swapKit', process.env.BUN_PUBLIC_SWAPKIT_API_KEY);
}
```

## Performance Tips

### Hot Module Replacement

Bun supports HMR out of the box. Use the `--hot` flag:

```bash
bun --hot src/server.tsx
```

### Bundle Optimization

Bun's bundler is highly optimized. For production builds:

```typescript
// @noErrorValidation
// build.ts with optimization
await build({
  entrypoints: ["./pages/index.tsx"],
  outdir: "./dist",
  target: "browser",
  minify: {
    whitespace: true,
    identifiers: true,
    syntax: true,
  },
  splitting: true,
  sourcemap: "external",
  // Tree-shake unused code
  treeShaking: true,
});
```

## Testing

Bun includes a built-in test runner:

```typescript
// @noErrorValidation
// src/App.test.tsx
import { expect, test } from "bun:test";
import { render } from '@testing-library/react';
import { App } from './App';

test("renders without crashing", () => {
  const { container } = render(<App />);
  expect(container).toBeDefined();
});
```

Run tests:

```bash
bun test
```

## Common Issues

### Module Resolution

Bun handles module resolution differently than Node.js. If you encounter issues:

1. Clear the cache: `bun clean`
2. Reinstall dependencies: `bun install`

### TypeScript Configuration

Bun supports TypeScript out of the box. Create a `tsconfig.json`:

```json
{
  "compilerOptions": {
    "target": "ESNext",
    "module": "ESNext",
    "jsx": "react-jsx",
    "moduleResolution": "bundler",
    "types": ["bun-types"]
  }
}
```

## Example Repository

For a complete working example, check out the [Bun + React playground](https://github.com/swapkit/swapkit/tree/develop/playgrounds/bun-react) in the SwapKit repository.






