...
For production applications, you'll want to add your own API keys:
...
const swapKit = createSwapKit({
  config: {
    apiKeys: {
      // For WalletConnect v2 support
      walletConnectProjectId: 'your-project-id',
      // For UTXO chains (Bitcoin, Litecoin, etc.)
      blockchair: 'your-blockchair-key',
      // For SwapKit API features
...
### 1. Chains
SwapKit supports 30+ blockchain networks with a unified chain identifier system:
...
const cosmos = Chain.Cosmos;       // "COSMOS"
const thorchain = Chain.THORChain; // "THOR"
const ripple = Chain.Ripple;       // "XRP"
const solana = Chain.Solana;       // "SOL"
const polkadot = Chain.Polkadot;   // "DOT"
const radix = Chain.Radix;         // "XRD"
```

### 2. Assets
Assets are identified by their chain and symbol:

```typescript
// @noErrorValidation
import { AssetValue } from '@swapkit/helpers';

// Native assets use the asset string format
const eth = AssetValue.from({ asset: 'ETH.ETH', value: 1 }); // 1 ETH
const btc = AssetValue.from({ asset: 'BTC.BTC', value: 0.1 }); // 0.1 BTC

// Token assets include the contract address
...
// Mobile wallets
const walletconnect = WalletOption.WALLETCONNECT;

// Hardware wallets
const ledger = WalletOption.LEDGER;
const trezor = WalletOption.TREZOR;
...
console.log(`Expected output: ${bestRoute.expectedOutput}`);
console.log(`Fees: ${bestRoute.totalFees}`);
...
2. **[Perform Swaps](/guides/actions/swap)** - Advanced swapping techniques
3. **[Create Custom Plugin](/guides/create-plugin)** - Extend SwapKit functionality
4. **[Framework Integration](/guides/integrations/nextjs)** - Use with Next.js, Vite, etc.
