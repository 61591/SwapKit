...
export function TokenBalance({ balance }: TokenBalanceProps) {
  ...
  return (
    <div className={`flex items-center justify-between p-3 rounded-lg border hover:bg-accent/50 transition-colors ${balance.isGasAsset ? 'bg-accent/25' : ''}`}>
      <div className="flex items-center gap-2">
        {iconUrl ? (
          ...
          <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium ${balance.isGasAsset ? 'bg-primary text-primary-foreground' : 'bg-accent'}`}>
